...
var Environment = (function () {
	...
	_createClass(Environment, null, [{
		...
		value: function resolveAll() {
			...
			for (var prop in node) if (node[prop] instanceof Object) node[prop] = this.resolveAll(node[prop]);else node[prop] = this.formatsValue(node[prop]);

			return node;
		}
	}, {
		key: 'formatsValue',
		value: function formatsValue() {
			...
			try {
				for (var _iterator = values[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
					...
					while (matched = RG_PARAM.exec(value)) {
						...
						if (value !== undefined) value = value.replace('${' + envVar + '}', value);
					}
				}
			} catch (err) {
